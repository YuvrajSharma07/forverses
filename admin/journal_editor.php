...
<html lang="en">
    <head>
        <meta charset="utf-8">
        <title>Upload Journal PDF</title>
    ...
    <body>
        ...
        if(isset($_FILES['attachment'])){
            ...
            if(in_array($file_type,$mime_types) === false){
                array_push($errors, "Extension not allowed, please choose a valid file.");
            }
            if(empty($errors) == true){
                move_uploaded_file($file_tmp,"../".$file_name);
                $db_connection = dbConnect('forverses');
                $name = mysqli_real_escape_string($db_connection, $file_name);
                $sql_update = "INSERT INTO journal (journal)
                VALUES ('".$name."')";
                if (!mysqli_query($db_connection, $sql_update)) {
                    echo "Error: ".$sql_update."<br>".mysqli_error($db_connection);
                }
